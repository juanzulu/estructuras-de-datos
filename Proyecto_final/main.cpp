  ...
  do {
    ...
    if (input != "salir") {
      ...
      if (tokens.size() == 1) {
        if (tokens[0] == "ayuda") {
          ayuda(tokens);
        } else if (tokens[0] == "ubicar_elementos" && ubicar_elementos(robot, arbol)) {
        ...
        if (tokens[0] == "ayuda") {
          ayuda(tokens);
        } else if (tokens[0] == "cargar_comandos") {
          ...
          crear_mapa(tokens[1], mapa, robot);
        }
        ...
        if (tokens[0] == "agregar_movimiento") {
          if (agregar_movimiento(robot, tokens[1], tokens[2], tokens[3])) {
            ...
            cout << "La información de movimiento no corresponde a los datos esperados(tipo,magnitud,unidad)" << endl;
          }
          ...
          if (agregar_analisis(robot, tokens[1], tokens[2], tokens[3])) {
            ...
            cout << "La información de análisis no corresponde a los datos esperados(tipo,objeto,comentario)" << endl;
          } 
        }
        ...
        if (tokens[0] == "agregar_elemento") {
          if (agregar_elemento(robot, tokens[1], tokens[2], tokens[3], tokens[4], tokens[5])) {
            ...
            cout << "La información de elemento no corresponde a los datos esperados(tipo,tamaño,unidad,x,y)" << endl;
          }
        }
      }
    }
