...
void imprimir(Curiosity &robot) {
  ...
  for (list<Analisis>::iterator it = robot.obtenerListaAnalisis().begin(); it != robot.obtenerListaAnalisis().end(); it++) {
  ...
  for (list<Movimiento>::iterator it = robot.obtenerListaMovimientos().begin(); it != robot.obtenerListaMovimientos().end();
       it++) {
  ...
  for (list<Elemento>::iterator it = robot.obtenerElementos().begin(); it != robot.obtenerElementos().end(); it++) {
...
bool guardar(Curiosity &robot, string tipoA, string nombre) {
  ...
  if (!archivo.is_open()) {
    ...
    if (tipoA == "comandos" && (robot.obtenerListaMovimientos().size() == 0 ||
      ...
      for (list<Analisis>::iterator it = robot.obtenerListaAnalisis().begin(); it != robot.obtenerListaAnalisis().end();
           it++) {
      ...
      for (list<Elemento>::iterator it = robot.obtenerElementos().begin(); it != robot.obtenerElementos().end();
           it++) {
...
bool ubicar_elementos(Curiosity &robot, KDTree &arbol) {
  ...
  for (list<Elemento>::iterator it = robot.getElementoBegin(); it != robot.getElementoEnd(); it++) {//Recorre la lista de elementos del curiosity
  ...
  if (!fallidos.empty()) {
    ...
    for(list<Elemento>::iterator it = fallidos.begin(); it != fallidos.end(); it++) {
...
list<pair<Elemento*, float>> elementos_cercanos(Elemento* elem, int n, list<Elemento*> elementos) {
    list<pair<Elemento*, float>> l_res;

    list<Elemento*> list_copy = elementos;
    ...
    for (Elemento* el: list_copy) {
        ...
        float dist_tmp = sqrt(
          pow((elem->obtenerCoordenada_x() - el->obtenerCoordenada_x()), 2)
          + pow((elem->obtenerCoordenada_y() - el->obtenerCoordenada_y()), 2)
        );
...
void crear_mapa(string coefstr, Grafo& mapa, Curiosity robot) {
    if (!esNumero(coefstr)) {
        cout << "El coeficiente de conectividad debe ser un numero\n";
        return;
    }
    
    if (robot.obtenerElementos().empty()) {
        cout << "La informacion requerida no esta almacenada en memoria.\n";
        return;   
    }
    
    float coef = stof(coefstr);

    if (coef < 0 || coef > 1) {
        cout << "El coeficiente de conectividad debe tener un valor entre 0 y 1\n";
        return;
    }

    mapa.borrar();

    int vecinos = round(coef * robot.obtenerElementos().size());

    if (vecinos == robot.obtenerElementos().size())
        vecinos--; //? no puede tenerse a si mismo como arista (1 arista menos del total como maximo)

    for (Elemento e: robot.obtenerElementos()) {
        mapa.InsVertice(new Elemento(e));
    }
    
    for (Elemento* el: mapa.getVertices()) {
        int v1 = mapa.idVertice(*el);

        for (pair<Elemento*, float> p: elementos_cercanos(el, vecinos, mapa.getVertices())) {
            int v2 = mapa.idVertice(*(p.first));

            mapa.InsArco(v1, v2, p.second);
        }
    }
    ...
    //? Para pruebas:
    // for (Elemento* e: mapa.getVertices()) {
    //     int id = mapa.idVertice(*e);
    //     cout << id;
    //     cout << " -> { ";
    //     for (int id_tmp: mapa.sucesores(id)) {
    //         cout << "(" << mapa.CostoArco(id, id_tmp) << ", " << id_tmp << "); ";
    //     }
    //     cout << "}\n";
    // }
}
// de 1 vertice a otro
pair<list<int>, float> ruta_mas_larga_elems_recur(int id1, int id2, list<int> visitados, Grafo mapa) {
    if (id1 == id2) {
        return make_pair(list<int>({id1}), 0);

    } else {
        visitados.push_back(id1);

        pair<list<int>, float> res = make_pair(list<int>(), -1);

        for (int i: mapa.sucesores(id1)) {
            if (find(visitados.begin(), visitados.end(), i) == visitados.end()) {
              // no lo encontro
                float costo_tmp = mapa.CostoArco(id1, i);

                pair<list<int>, float> tmp = ruta_mas_larga_elems_recur(i, id2, visitados, mapa);
                tmp.second += costo_tmp;

                if (costo_tmp > res.second) {
                    res = tmp;
                    res.first.push_front(id1);
                }
            }
        }
        return res;
...
// 1 vertice a el resto
pair<list<int>, float> ruta_mas_larga_elem(int id, Grafo mapa) {

    if (id >= mapa.OrdenGrafo()) {
        throw runtime_error("El elemento no se encuentran en el mapa");
    }

    pair<list<int>, float> res_ids = make_pair(list<int>(), -1);
    for (int i = 0; i < mapa.OrdenGrafo(); i++) {
        pair<list<int>, float> tmp = ruta_mas_larga_elems_recur(id, i, list<int>(), mapa);
...
// de todos los vertices a todos
void ruta_mas_larga(Grafo mapa) {

    if (mapa.OrdenGrafo() == 0) {
        cout << "El mapa no ha sido generado todavia (con el comando crear_mapa))\n";
        return;
    }
    ...
    for (int i = 0; i < mapa.OrdenGrafo(); i++) {
        pair<list<int>, float> tmp = ruta_mas_larga_elem(i, mapa);
    ...
    cout << "Los puntos de interes mas alejados entre si son: '" << mapa.InfoVertice(*(res.first.begin()))->toString() << "' y '" << mapa.InfoVertice(*(res.first.rbegin()))->toString() << "'\n";
    ...
    for (int id: res.first) {
        cout << "'" << mapa.InfoVertice(id)->toString() << "'\n";
    }
}